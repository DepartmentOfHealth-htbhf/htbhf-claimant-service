...
public interface PaymentCycleRepository extends CrudRepository<PaymentCycle, UUID> {
    ...
    @Query(nativeQuery = true, value =
            "SELECT Cast(claim_id as varchar) as claimIdString, cycle_end_date as cycleEndDateTimestamp FROM "
                    + "(SELECT DISTINCT ON (c.id) c.id as claim_id, p.cycle_start_date, p.cycle_end_date "
